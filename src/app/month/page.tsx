...
import { useMonthData } from '@/libs/DataSource';
...
export default function MonthPage() {
  ...
  const parsedDay = urlMonth ? dayjs(urlMonth, 'YYYY-MM', true) : undefined;
  const urlDate = parsedDay?.isValid() ? parsedDay : dayjs().startOf('month');

  const { data, isLoading, error } = useMonthData(urlDate.format('YYYY-MM'));
  ...
  const handleDateChange = (newDate: dayjs.Dayjs | null) => {
    if (newDate) {
      const formatted = newDate.format('YYYY-MM');
      router.push(`${pathname}?m=${formatted}`);
    ...
    <>
      ...
      <Stack>
        <Stack direction="row" width="100%" marginY={2} alignItems="center">
          ...
          <IconButton
            onClick={() => {
              const previous = urlDate.subtract(1, 'month').format('YYYY-MM');
              router.push(`${pathname}?m=${previous}`);
            }}
          >
            <ArrowLeftIcon />
          </IconButton>
          ...
          >
            <DateCalendar value={urlDate} onChange={handleDateChange} views={['month', 'year']} disableFuture />
          </Popover>
          ...
          <IconButton
            onClick={() => {
              const next = urlDate.add(1, 'month').format('YYYY-MM');
