...
import { useYearData } from '@/libs/DataSource';
...
export default function YearPage() {
  ...
  const parsedDay = urlYear ? dayjs(urlYear, 'YYYY', true) : undefined;
  const urlDate = parsedDay?.isValid() ? parsedDay : dayjs().startOf('year');

  const { data, isLoading, error } = useYearData(urlDate.format('YYYY'));
  ...
  const handleDateChange = (newDate: dayjs.Dayjs | null) => {
    if (newDate) {
      const formatted = newDate.format('YYYY');
      router.push(`${pathname}?y=${formatted}`);
    ...
    <>
      ...
      <Stack>
        <Stack direction="row" width="100%" marginY={2} alignItems="center">
          ...
          <IconButton
            onClick={() => {
              const previous = urlDate.subtract(1, 'year').format('YYYY');
              router.push(`${pathname}?y=${previous}`);
            }}
          >
            <ArrowLeftIcon />
          </IconButton>
          ...
          >
            <DateCalendar value={urlDate} onChange={handleDateChange} views={['year']} disableFuture />
          </Popover>
          ...
          <IconButton
            onClick={() => {
              const next = urlDate.add(1, 'year').format('YYYY');
              router.push(`${pathname}?y=${next}`);
            }}
          >
            <ArrowRightIcon />
          </IconButton>
